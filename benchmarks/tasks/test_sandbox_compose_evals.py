...
def test_every_tmpfs_entry_carries_explicit_size_cap(compose: dict) -> None:
    ...
    for path, opts in entries.items():
        ...
        assert match is not None, (
            f"tmpfs {path!r} must declare an explicit size= cap (issue #123); " f"got opts={opts!r}"
        )
...
def test_pids_limit_matches_deploy_resources_pids(compose: dict) -> None:
    ...
    assert top_level == _EXPECTED_PIDS_LIMIT, (
        f"pids_limit must be {_EXPECTED_PIDS_LIMIT} per issue #118; " f"got {top_level}"
    )
...
def test_capabilities_dropped_and_no_new_privileges(compose: dict) -> None:
    ...
    assert any(opt in {"no-new-privileges", "no-new-privileges:true"} for opt in normalised), (
        f"security_opt must include 'no-new-privileges:true' (issue #124); " f"got {security_opt!r}"
    )
