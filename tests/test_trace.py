...
# --- ModelConfig / model_id plumbing (issue #361) -------------------------------


@_BACKENDS
def test_critic_verdict_joins_to_session_model_id(tmp_path, backend):
    """Issue #361 acceptance: a critic_verdict event can be joined to its
    session's model_id.

    The Phase 3 improvement-rate KPI requires attributing benchmark outcomes
    to specific quantizations. This test verifies the join path works:
    session -> list_sessions() -> model_id, and event -> session_id ->
    list_sessions() -> model_id.
    """
    ...
    with logger.session(harness_version="0.1.0", model_config=model_config) as sid:
        logger.record(
            sid,
            kind="critic_verdict",
            payload={"approved": True, "passed_checks": ["pytest"]},
        )
    ...
    events = logger.load_session(sid)
    assert len(events) == 1
    assert events[0].kind == "critic_verdict"
    assert events[0].session_id == sid

    session_for_event = next(s for s in sessions if s.session_id == events[0].session_id)
    assert session_for_event.model_id == "codellama-7b.Q5_K_M.gguf"


@_BACKENDS
def test_session_model_id_from_bare_string_still_works(tmp_path, backend):
    """Backward compatibility: passing model_id as a string still works."""
    path = tmp_path / f"traces{_suffix(backend)}"
    logger = TraceLogger(path, backend=backend)
