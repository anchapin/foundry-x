...
def test_rocm_setup_help_exits_zero() -> None:
    ...
    assert "usage: rocm_setup.sh [--check-rocm] [--check-gpu-responsive] [--smoke-test <gguf>]" in result.stdout
...
# --- Git-ref-aware tests (issue #638) ---


def test_rocm_setup_same_ref_skips_fetch_checkout(tmp_path: Path) -> None:
    """When HEAD already matches LLAMACPP_REF, fetch+checkout is skipped."""
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    llamacpp_dir = tmp_path / "llama.cpp"
    server_bin = llamacpp_dir / "build" / "bin" / "llama-server"
    server_bin.parent.mkdir(parents=True)
    fetch_marker = tmp_path / "fetch_called"
    rev_parse_marker = tmp_path / "rev_parse_ref"
    model_file = tmp_path / "model.gguf"
    model_file.write_text("fake model content", encoding="utf-8")
...
""",
    ...
    # Write the current ref (same as target) so rev-parse returns the matching ref
    rev_parse_marker.write_text(f"{target_ref}\n", encoding="utf-8")
    ...
    assert not fetch_marker.exists(), "fetch should not have been called when ref matches"


def test_rocm_setup_different_ref_triggers_fetch_checkout(tmp_path: Path) -> None:
    """When HEAD differs from LLAMACPP_REF, fetch+checkout is triggered."""
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    llamacpp_dir = tmp_path / "llama.cpp"
    server_bin = llamacpp_dir / "build" / "bin" / "llama-server"
...
""",
    ...
    # Write a different current ref so it doesn't match target
    rev_parse_marker.write_text(f"{current_ref}\n", encoding="utf-8")
    ...
    assert fetch_marker.exists(), "fetch should have been called when ref differs"
