...
if [[ -n "${LLAMACPP_SKIP_BUILD:-}" && -x "$SERVER_BIN" ]]; then
    echo "Skipping build (LLAMACPP_SKIP_BUILD set and binary exists)"
else
...
# Assert GPU is active when n-gpu-layers > 0 (issue #815).
