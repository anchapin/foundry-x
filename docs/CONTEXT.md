...
| **`session_evolved`** | `TraceLogger.mark_session_evolved` (`src/foundry_x/trace/logger.py`) | JSONL marker line `{"session_id", "kind": "session_evolved"}`. In sqlite it updates the `sessions.evolved` column (0→1). Idempotent: appending twice or calling twice has no additional effect. Used by `run_evolution_daemon` (issue #1047) to prevent reprocessing. | no |
...
| **`model_cost`** | `Runner.run_task` via `_on_cost` closure wired onto `CloudModelAdapter.on_cost` (issues #1041, ADR-0029) | `{"provider": str, "model": str, "prompt_tokens": int, "completion_tokens": int, "estimated_cost_usd": float, "pricing_known": bool}` — emitted on each successful model response; cost is computed from the provider's per-token pricing table (`token_pricing`) and reported `ModelUsage`. Feeds the improvement-rate KPI's cost attribution so the Evolver can reason about spend vs. harness quality, not just model price. `pricing_known` (issue #1465) is `false` when the model is absent from the pricing table and no `FOUNDRY_MODEL_PRICING_*` env override applies; in that case `estimated_cost_usd` is `0.0` as a placeholder, distinguishing "pricing unknown" from "genuinely free" so KPI consumers can tell which sessions carry reliable cost attribution. Only emitted by `CloudModelAdapter` subclasses (not `OpenAICompatibleAdapter`). | no |
| **`model_rate_limit`** | `Runner.run_task` via `_on_rate_limit` closure wired onto `CloudModelAdapter.on_rate_limit` (issues #1041, ADR-0029) | `{"requests_remaining": int \| null, "tokens_remaining": int \| null, "requests_reset_seconds": float \| null, "tokens_reset_seconds": float \| null}` — provider rate-limit window snapshot parsed from response headers. Anthropic surfaces ``anthropic-ratelimit-requests-remaining`` / ``anthropic-ratelimit-tokens-remaining``; OpenAI surfaces ``x-ratelimit-remaining-requests`` / ``x-ratelimit-remaining-tokens``. Adapters normalise both into this single shape so the trace store can reason about headroom without branching on provider. Only emitted by `CloudModelAdapter` subclasses (not `OpenAICompatibleAdapter`). | no |
...
- **`FAILURE_KINDS`** (constant in
  `src/foundry_x/evolution/digester.py:122-160`): `tool_error`,
...
- **`FAILURE_PAYLOAD_KEYS`** (constant in
  `src/foundry_x/evolution/digester.py:162-168`): `error`, `traceback`,
