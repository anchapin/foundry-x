...
| **`model_cost`** | `Runner.run_task` via `_on_cost` closure wired onto `CloudModelAdapter.on_cost` (issues #1041, ADR-0029) | `{"provider": str, "model": str, "prompt_tokens": int, "completion_tokens": int, "estimated_cost_usd": float}` — emitted on each successful model response; cost is computed from the provider's per-token pricing table (`token_pricing`) and reported `ModelUsage`. Feeds the improvement-rate KPI's cost attribution so the Evolver can reason about spend vs. harness quality, not just model price. Only emitted by `CloudModelAdapter` subclasses (not `OpenAICompatibleAdapter`). | no |
